...
//
// Все проёмы в стенках подняты ВЫШЕ самого высокого компонента платы:
// держатель предохранителя уходит внутрь корпуса на 31 мм, гермовводы — на
// 10–15 мм, и на прежней высоте они упирались бы в реле и сетевой модуль.
//
...
board_x   = 150;   // плата 10x15 = 58x38 отверстий = 145x94 мм по крайним пятачкам
board_y   = 100;
board_t   = 1.6;
...
// Пересчёт координат платы (колонка/ряд из board.json) в координаты корпуса
function bx(col) = main_off[0] + (col - 1) * 2.54;
function by(row) = main_off[1] + (row - 1) * 2.54;
...
// Стойки платы: крепёжные отверстия [2,2],[57,2],[2,37],[57,37]
main_holes = [ for (xy = [[2,2],[57,2],[2,37],[57,37]]) [bx(xy[0]), by(xy[1])] ];

// ---------- проёмы в стенках ----------
// Зона 220 на плате — ряды 27..38, то есть у ДАЛЬНЕЙ стенки (y = in_y).
// Туда же выходят оба гермоввода и держатель предохранителя.
pg9_d       = 15.2;   // гермоввод PG9
fuse_hole_d = 13.2;   // резьба M13 держателя предохранителя

gland_in_x  = 42;     // ввод 220 из щитка -> клеммник ACIN
fuse_x      = 64;     // держатель предохранителя (фаза проходит через него)
gland_out_x = 86;     // кабель к приводу с клеммника ACOUT
...
module body() {
  difference() {
    ...
    hole_far(fuse_x,      pen_z, fuse_hole_d);  // держатель предохранителя
