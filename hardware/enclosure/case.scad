// ============================================================================
// Умные Ворота — корпус под 3D-печать (ревизия r2, низкий профиль)
// ============================================================================
// Компоновка (см. hardware/board/board.json):
//   одна плата 10x15 на стойках, сетевой AC-DC модуль (HLK-15M12) стоит на ней.
...
// Основная плата 10x15 — единственная (ревизия r2: AC-DC модуль на плате,
// этажерка упразднена). Модуль HLK-15M12 высотой 22 мм стоит сверху платы.
main_x    = 150;
main_y    = 100;

in_x = 160;
in_y = 110;
// Высота: стойки платы 8 (под платой разводка) + плата 1.6 + модуль 24 + воздух 12
standoff  = 8;
in_z      = standoff + 1.6 + 24 + 12;   // 45.6 -> печатаем 46

// Позиции платы во внутренних координатах
main_off  = [(in_x - main_x)/2, (in_y - main_y)/2];   // [5, 5]
...
fuse_hole_d   = 13.2;
fuse_center_z = floor_t + 22;
fuse_center_x = main_off[0] + 23*2.54;  // нижняя стенка (Y=0), напротив клеммника ACIN

// Гермовводы PG9: отверстие 15.2
pg9_d      = 15.2;
gland_z    = floor_t + 16;
gland1_x   = main_off[0] + 12*2.54;     // ввод 220 (нижняя стенка, зона реле/220)
gland2_y   = 40;                        // кабель привода (левая стенка)

// SMA-гнёзда: отверстие 6.5. Высота — уровень верхней платы.
sma_d      = 6.5;
sma_z      = standoff + 1.6 + 8;
...
module body() {
  ...
  // --- стойки платы ---
  for (h = main_holes)
    translate([wall+h[0], wall+h[1], floor_t]) boss(standoff, insert_d);
